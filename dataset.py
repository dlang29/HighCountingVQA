...
class HighCountVQADataset(Dataset):
  ...
  def __getitem__(self, idx):
    ...
    if "blip2" in config.MODEL_ID:
      question = "Question: " + data_point['question'] + " Answer:"
    else:
...
class HFHighCountVQADataset(HighCountVQADataset):
  ...
  def __getitem__(self, idx):
    ...
    if "pali" in config.MODEL_ID:
      ...
      # NOTE: set label padding positions to -100 in order to be ignored for the loss computation (throws error and no fix found...)
