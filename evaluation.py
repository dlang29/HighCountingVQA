...
np.savez('./data/evaluation/blip.npz', bin_count=bin_count, bin_acc=bin_acc, bin_avg_abs_err=bin_avg_abs_err, nan_count=nan_count)
...
create_bar_plots(x=x, Y=np.vstack((bin_count, bin_count)), labels=label_dict, save_name="distribution.png")

label_dict.update({'y_label': 'Accuracy', 'title': 'Accuracy over individual numbers of objects'})
create_bar_plots(x=x, Y=np.vstack((bin_acc, bin_acc)), labels=label_dict, save_name="accuracy.png")
...
create_dual_axis_bar_plots(x=x, Y=np.vstack((blip_dual_data, blip_dual_data)), labels=label_dict, save_name="abs_error.png")
